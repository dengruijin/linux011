...
该函数执行过程如下：(sh_bang初值=0)
1. 检查是否有权限执行该文件,若无权限则返回错误
2. 取出执行文件头部数据,判断是否为脚本文件(文件开头为'#!'就认为是脚本文件)  
  3. 如果是脚本文件则读取与`#!`同一行的脚本名称(如/bin/bash)和参数(如果有)。然后调用`copy_string()`将`argv`和`envp`指定的用户空间字符串拷贝到新申请的主内存中, 拷完之后再把`#!`后面指定的脚本名称和参数(如果有)放到`argv`和`envp`的前面(主内存区)。
